...
enum mallQuests // EXTINCT
{
    WELCOME_TO_PROJECT_EXTINCT    = 50000,
    ENRICH_ME_MACHINE            = 50001,
    GNOME_EATER                    = 50002,
    IM_MY_OWN_BEST_FRIEND        = 50003,
    CHASIN_THE_BASIN            = 50004,
    THROUGH_THE_NETHER            = 50005,
    TUNNEL_DEFENDER                = 50006,
    THEY_SAID_ID_NEVER_GET        = 50007,
    GHOAT_RIDER                    = 50008,
};

enum Marks // EXTINCT
{
    MARK_ALTERAC_VALLEY    =    20560,
    MARK_WARSONG_GULCH    =    20558,
    MARK_ARATHI_BASIN    =    20559,
    MARK_EOTS            =    29024,
    MARK_SOTA            =    42425,
    MARK_IOC            =    47395,
    MARK_OF_HONOR        =    500000,
};
...
void Player::SetGameMaster(bool on, bool dev)
{
    if (on)
    {
...
void Player::RepopAtGraveyard()
{
    ...
    if (zone->ID == 616)
        TeleportTo(1, 4830.419922f, -1962.310059f, 1071.418091f, 3.310070f); // Extinct - Die in duelzone? Repop There!
    else if (zone->ID == 41)
        TeleportTo(0,-11138.699219f,-1748.891357f,-29.736399f,1.289355f); // Extinct - Die at mall? Repop There!
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (IsActiveQuest(70000)) //EXTINCT Q 500 HK's (donor tier 1)
            {
                if (GetSession()->GetSecurity() >= SEC_DONOR_ONE) // prevent non-donors from completing this quest.
                {
                    if (GetUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS) >= 500)
                        CompleteQuest(70000);
                }
            }
        }
    ...
    if (InBattleground() && honor > 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
                uint32 sourcemap = GetMapId();
                    {
                        switch (sourcemap) // EXTINCT
                        {
                        case MAP_ALTERAC_VALLEY:
                            if (GetQuestStatus(IM_MY_OWN_BEST_FRIEND) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_OF_HONOR,1);
                            if (GetQuestStatus(GHOAT_RIDER) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_ALTERAC_VALLEY,1);
                            break;
                        case MAP_WARSONG_GULCH:
                            if (GetQuestStatus(IM_MY_OWN_BEST_FRIEND) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_OF_HONOR,1);
                            if (GetQuestStatus(TUNNEL_DEFENDER) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_WARSONG_GULCH,1);
                            break;
                        case MAP_ARATHI_BASIN:
                            if (GetQuestStatus(IM_MY_OWN_BEST_FRIEND) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_OF_HONOR,1);
                            if (GetQuestStatus(CHASIN_THE_BASIN) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_ARATHI_BASIN,1);
                            break;
                        case MAP_EOTS:
                            if (GetQuestStatus(IM_MY_OWN_BEST_FRIEND) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_OF_HONOR,1);
                            if (GetQuestStatus(THROUGH_THE_NETHER) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_EOTS,1);
                            break;
                        case MAP_SOTA:
                            if (GetQuestStatus(IM_MY_OWN_BEST_FRIEND) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_OF_HONOR,1);
                            if (GetQuestStatus(THEY_SAID_ID_NEVER_GET) == QUEST_STATUS_INCOMPLETE)
                                AddItem(MARK_SOTA,1);
                            break;
                        case MAP_IOC:
                            if (GetQuestStatus(IM_MY_OWN_BEST_FRIEND) == QUEST_STATUS_INCOMPLETE)
...
void Player::DuelComplete(DuelCompleteType type)
{
    ...
    // Victory emote spell & anti-death
    if (type != DUEL_INTERRUPTED && duel->opponent)
        duel->opponent->CastSpell(duel->opponent, 52852, true); //cheer effect
    ...
    // Revive loser, we can't do a die Check because this happens to fast before the player is flagged as JUST_DIED
    duel->initiator->ResurrectPlayer(1.0f, false);
    duel->initiator->SpawnCorpseBones();
    duel->initiator->SaveToDB();

    // Revive winner, we can't do a die Check because this happens to fast before the player is flagged as JUST_DIED
    duel->opponent->ResurrectPlayer(duel->opponent->GetSession()->GetSecurity() > SEC_PLAYER ? 1.0f : 0.5f);
    duel->opponent->SpawnCorpseBones();
    duel->opponent->SaveToDB();
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    map = sMapMgr->CreateMap(mapId, this, instanceId); //exploit fix
    SetMap(map);
    ...
    if (GetSession()->GetSecurity() > SEC_PLAYER)
    {
        switch (sWorld->getIntConfig(CONFIG_GM_LOGIN_STATE))
        {
            ...
            case 1: SetGameMaster(true,false); break;             // enable
            case 2:                                         // save state
...
void Player::UnlearnTalentsYouShouldNotHave()
{
                for (uint32 i = 0; i < sTalentStore.GetNumRows(); ++i)
                {
                    TalentEntry const *talentInfo = sTalentStore.LookupEntry(i);
                    ...
                    for (int8 rank = MAX_TALENT_RANK-1; rank >= 0; --rank)
                    {
                        ...
                        if (talentInfo->RankID[rank] == 0)
                            continue;
                    // if the talent can be found in the PlayerTalentMap
                    if (!HasTalent(talentInfo->RankID[rank], m_activeSpec)) // als player talent niet heeft.
                    {
                        removeSpell(talentInfo->RankID[rank], true, false); // removes the talent, and all dependant, learned, and chained spells..
                        if (const SpellEntry *_spellEntry = sSpellStore.LookupEntry(talentInfo->RankID[rank]))
                            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)                  // search through the SpellEntry for valid trigger spells
                                if (_spellEntry->EffectTriggerSpell[i] > 0 && _spellEntry->Effect[i] == SPELL_EFFECT_LEARN_SPELL)
                                    removeSpell(_spellEntry->EffectTriggerSpell[i], true); // and remove any spells that the talent teaches
...
void Player::RewardDonator()
{
    uint32 security = GetSession()->GetSecurity();

    if (security > SEC_PLAYER)
    {
        CharTitlesEntry const* titleTenStorms = sCharTitlesStore.LookupEntry(86); // <Name> of the Ten Storms
        CharTitlesEntry const* titleSeeker = sCharTitlesStore.LookupEntry(81); // <Name> the Seeker

        switch(security)
        {
        case SEC_DONOR_ONE:
            if (!HasTitle(titleSeeker))
                SetTitle(titleSeeker);
            if (!HasSpell(41252)) // raven lord
                learnSpell(41252, false);
        break;
        case SEC_DONOR_TWO:
            if (!HasTitle(titleTenStorms))
                SetTitle(titleTenStorms);
            if (!HasTitle(titleSeeker))
                SetTitle(titleSeeker);
            if (!HasSpell(42777)) // spectral tiger
                learnSpell(42777, false);
        break;
        }
    }
}

void Player::RewardSpecialty()
{
    CharTitlesEntry const* titleStalker = sCharTitlesStore.LookupEntry(91); // Stalker <Name>
    CharTitlesEntry const* titleSupreme = sCharTitlesStore.LookupEntry(85); // <Name> the Supreme

    uint32 accountid = GetSession()->GetAccountId();

    QueryResult result = LoginDatabase.PQuery("SELECT Specialty FROM specialty WHERE AccountID = '%u'", accountid);
    {
        if (result)
            {
                Field* fields = result->Fetch();
                uint32 specialty = fields[0].GetUInt32();
                ...
                break;
                case 1: //pre launch
                    SetTitle(titleStalker);
                break;
                case 2: //tester
                    SetTitle(titleStalker);
                    SetTitle(titleSupreme);
                    learnSpell(25849, false); // Baby Shark
                break;
                }
            }
    }
...
{
    SPELL_GENERIC                    = 2382,
    SPELL_GENERIC_DUEL                = 7266,
    SPELL_GENERIC_TRINKETS            = 9125,
    SPELL_GENERIC_ATTACKING            = 8386,
    SPELL_GENERIC_DETECT            = 3050,

    SPELL_GENERIC_OPENING1            = 6247,
    SPELL_GENERIC_OPENING2            = 6477,
    SPELL_GENERIC_OPENING3            = 6478,
    SPELL_GENERIC_OPENING4            = 3365,
    SPELL_GENERIC_OPENING5            = 21651,
    SPELL_GENERIC_OPENING6            = 61437,

    SPELL_GENERIC_OPENING_NO_TEXT    = 22810,

    SPELL_GENERIC_CLOSING1            = 6233,
    SPELL_GENERIC_CLOSING2            = 6246,
    SPELL_GENERIC_CLOSING3            = 21652,

    SPELL_GENERIC_UNARMED            = 203,
    SPELL_GENERIC_DISARM1            = 1843,

    SPELL_HONORLESS_TARGET            = 2479,

    SPELL_GENERIC_AUTO_UNSTUCK        = 7355,
    SPELL_GENERIC_GROVEL            = 7267,
    SPELL_GENERIC_INSIGNIA            = 22027,

    SPELL_2H_AXES                    = 197,
    SPELL_2H_SWORDS                    = 202,
    SPELL_SIGILS                    = 52665,
    SPELL_CHAOS_BOLT_PASSIVE        = 58284,
    SPELL_DEMONIC_IMMOLATE            = 75445,
    SPELL_FLAME_SHOCK_PASSIVE        = 75461,
    SPELL_RUNE_STRIKE_PROCC            = 56816,
};

void Player::LearnExtraSpells()
{
    switch(getClass())
    {
    case CLASS_DEATH_KNIGHT:
        if (!HasSpell(SPELL_GENERIC_DUEL))
        {
            learnSpell(SPELL_GENERIC_DUEL, false); //duels
            learnSpell(SPELL_GENERIC_TRINKETS, false); //trinkets
        }
        if (!HasSpell(SPELL_GENERIC))
        {
            learnSpell(SPELL_GENERIC, false);
            learnSpell(SPELL_GENERIC_ATTACKING, false); //something with attack-opening??
            learnSpell(SPELL_GENERIC_DETECT, false); // Basic Stealth Detection !!!

            learnSpell(SPELL_GENERIC_OPENING1, false); //opening doors
            learnSpell(SPELL_GENERIC_OPENING2, false); //opening ???
            learnSpell(SPELL_GENERIC_OPENING3, false); //opening ???
            learnSpell(SPELL_GENERIC_OPENING4, false); //opening chests
            learnSpell(SPELL_GENERIC_OPENING5, false); //opening ???
            learnSpell(SPELL_GENERIC_OPENING6, false); //opening ???

            learnSpell(SPELL_GENERIC_OPENING_NO_TEXT, false); //something with text after opening GObjects

            learnSpell(SPELL_GENERIC_UNARMED, false); // Used by knuckleduster achievement.
            learnSpell(SPELL_GENERIC_DISARM1, false); // Disarming bombs/things in IoC

            learnSpell(SPELL_HONORLESS_TARGET, false); //Honorless target when changing maps.

            learnSpell(SPELL_GENERIC_CLOSING1, false); //closing ???
            learnSpell(SPELL_GENERIC_CLOSING2, false); //closing ???
            learnSpell(SPELL_GENERIC_CLOSING3, false); //closing ???

            learnSpell(SPELL_2H_AXES, false); //2h axes skill :)
            learnSpell(SPELL_2H_SWORDS, false); //2h swords skill
            learnSpell(SPELL_SIGILS, false); //sigils
            learnSpell(SPELL_GENERIC_AUTO_UNSTUCK, false); //auto-unstuck
            learnSpell(SPELL_GENERIC_GROVEL, false); //???
            learnSpell(SPELL_GENERIC_INSIGNIA, false); // remove insignia's in BGs
            learnSpell(SPELL_RUNE_STRIKE_PROCC, false); // Fix runestrike proc
        }
        break;
    case CLASS_WARLOCK:
        if (!HasSpell(SPELL_CHAOS_BOLT_PASSIVE))
        {
            learnSpell(SPELL_CHAOS_BOLT_PASSIVE, false); //Chaos Bolt Passive, fixes absorb penetration.
            learnSpell(SPELL_DEMONIC_IMMOLATE, false); //Demonic Immolate, allow immolate dot to crit.
        }
    break;
    case CLASS_SHAMAN:
        if (!HasSpell(SPELL_FLAME_SHOCK_PASSIVE))
            learnSpell(SPELL_FLAME_SHOCK_PASSIVE, false); //Flame Shock Passive, allow flame shock dot to crit.
    break;
    default:
        break;
    }
