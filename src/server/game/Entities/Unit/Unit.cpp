...
void Unit::Update(uint32 p_time)
{
	...
	if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_TRUESIGHT))
		if (!HasAura(18950))
			CastSpell(this,18950,true); // anti-stealth eye
...
void Unit::DealDamageMods(Unit *pVictim, uint32 &damage, uint32* absorb)
{
    ...
    if (pVictim != this && IsControlledByPlayer() && pVictim->IsControlledByPlayer())
    {
        ...
        if (area && area->IsSanctuary() && (!area->ID == 616))     //sanctuary EXTINCT
        {
            if (absorb)
                *absorb += damage;
            damage = 0;
...
void Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo, bool durabilityLoss)
{
    ...
    if (pVictim != this && IsControlledByPlayer() && pVictim->IsControlledByPlayer())
    {
        ...
        if (area && area->IsSanctuary() && (!area->ID == 616))       // sanctuary
            return;
    }
...
{
	if (spellProto && spellProto->Attributes & SPELL_ATTR0_IMPOSSIBLE_DODGE_PARRY_BLOCK) //overpower fix extinct
        return false;
